...
async function showListWithLink(listId) {
  ...
  const listUrl = `${location.origin}${location.pathname}?list=${listId}`;
...
function showRecipient(data, docId) {
  ...
  if (storedId === docId) {
    ...
    db.collection(`lists/${currentListId}/names`)
      .doc(docId)
      .update({ pinViewed: true });
...
async function loadList(listId) {
  try {
    ...
    if (clickedNameId) {
      ...
      const clickedDoc = namesSnapshot.docs.find(
        (doc) => doc.id === clickedNameId
      );
      if (clickedDoc) {
